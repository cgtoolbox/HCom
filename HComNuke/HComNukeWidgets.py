...
class SendingDataThread(QtCore.QThread):
    ...
    def run(self):
        ...
        if self.imagePath:
            ...
            result = self.workFunc(self.target_clientID, self._sender, self.tabTarget, alembic=True, frames=self.frames)
...
class UserChatTabWidget(QtGui.QWidget):
    ...
    def __init__(self, target, clientType, openChatRoom=False, parent=None):
        ...
        self.sendAlembicBtn.setToolTip("Send Alembic mesh")
        ...
        self.sendAlembicBtn.clicked.connect(lambda: self.mainUI._sendAlembicMesh(self.target, self.tabTargetID, self))
        self.actionButtonLayout.addWidget(self.sendAlembicBtn)
        self.widgetList.append(self.sendAlembicBtn)
        
        self.sendOutputNodes = QtGui.QPushButton("")
        self.sendOutputNodes.setToolTip("Send node output as picture")
        self.sendOutputNodes.setIconSize(QtCore.QSize(32,32))
        self.sendOutputNodes.setFixedSize(40,40)
        self.sendOutputNodes.setIcon(QtGui.QIcon(ICONPATH + "sendoutputnode.png"))
        self.sendOutputNodes.setDisabled(not self.connected)
        self.sendOutputNodes.clicked.connect(lambda: self.mainUI._sendOuputNode(self.target, self.tabTargetID, self))
        self.actionButtonLayout.addWidget(self.sendOutputNodes)
        self.widgetList.append(self.sendOutputNodes)
    ...
    def appendMessage(self, header, message, fromMyself=False):
        ...
        if header:
            ...
            if fromMyself:
                timeStamp = HComNukeUtils.coloredString(timeStamp, "70738c", italic=True)
    ...
    def appendDataSendBox(self, msg, targets, _sender, tabTarget, workFunc, imagePath=None, alembic=False, frames=[0,0]):
        
        box = SendingDataMessageBox(msg, targets, _sender, tabTarget, workFunc, imagePath=imagePath, alembic=alembic, frames=frames, parent=self)
...
class UserListDockWidget(QtGui.QWidget):
    ...
    def _openReceivedFilesFolder(self):
        ...
        RECEIVED_FILES_PATH = HComNukeUtils.fetchMyReceivedFilesFolder()
...
class SettingsWindow(QtGui.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        initValues = HComNukeUtils.readIni()
        ...
        imageFormatLayout = QtGui.QHBoxLayout()
        imageFormatLayout.setSpacing(10)
        
        imageFormatLayout.addWidget(QtGui.QLabel("Node's output format:"))
        
        self.outputImageFormat = QtGui.QComboBox()
        self.outputImageFormat.addItems(["png", "jpg", "tga", "exr"])
        self.outputImageFormat.setCurrentIndex(self.outputImageFormat.findText(str(initValues["OUTPUT_IMAGE_FORMAT"])))
        imageFormatLayout.addWidget(self.outputImageFormat)
        
        settingsLayout.addItem(imageFormatLayout)
    ...
    def returnToDefault(self):
        ...
        self.outputImageFormat.setCurrentIndex(0)
    ...
    def validSettings(self):
        ...
        self.SETTINGS["OUTPUT_IMAGE_FORMAT"] = str(self.outputImageFormat.currentText())
        
        HComNukeUtils.writeIni(self.SETTINGS)
        self.close()
...
class SendingDataMessageBox(QtGui.QWidget):
    ...
    '''
    def __init__(self, msg, target_clientID, _sender, tabTarget, workFunc, imagePath=None, alembic=False, frames=[0,0], parent=None):
        QtGui.QWidget.__init__(self, parent=parent)
        ...
        self.workerThread.alembic = alembic
        self.workerThread.frames = frames
...
class MessageBox(QtGui.QWidget):
    ...
    def cancelInput(self):
        ...
        HComNukeClient.sendDataReceivedInfo(self.sender, self.mainUi.ID, [False, self.dataType], self.mainUi.ID)
        
    def acceptInput(self):
        from HComNukeClient import HComNukeUtils
        ...
        if self.dataType == "otl":
            ...
            self.workThread.workFonc = HComNukeUtils.createOtl
            self.workThread.start()
                
        # Bgeo mesh
        elif self.dataType == "mesh":
            
            self.workThread.workFonc = HComNukeUtils.createMesh
            self.workThread.start()
     
        # Pictures
        elif self.dataType == "pic":
            
            self.workThread.workFonc = HComNukeUtils.createPic
            self.workThread.start()
            
        # Alembic
        elif self.dataType == "alembic":
            
            self.workThread.workFonc = HComNukeUtils.createAlembic
            self.workThread.start()
    ...
    def endJob(self, result=None):
        ...
        HComNukeClient.sendDataReceivedInfo(self.sender, self.mainUi.ID, [True, self.dataType], self.mainUi.ID)
